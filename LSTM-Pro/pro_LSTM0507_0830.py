...
def prepareData(com_seq, incom_seq, protein):
    ...
    pairs = [pair for pair in pairs if (len(pair[0]) < MAX_LENGTH and len(pair[1]) < MAX_LENGTH) and (len(pair[0]) > MIN_LENGTH and len(pair[1]) > MIN_LENGTH) and (len(pair[2])<MAX_PROTEIN_LENGTH)]
    print('filtrate to {} sentence pairs'.format(len(pairs)))
    ...
    print(input_seq.name, input_seq.n_chars)
    print(output_seq.name, output_seq.n_chars)
    print(protein_seq.name, protein_seq.n_chars)
...
print(input_seq.index2char)
print(output_seq.index2char)
print(protein_seq.index2char)
...
def showPlot(loss, axix):
    ...
    plt.show()
    #plt.close()
    plt.savefig('results/LSTM-loss0530')
    plt.close()
...
def showPlot_val(loss, val_loss, axix):
    ...
    plt.ylabel('Loss')
    plt.show()
    plt.savefig('results/LSTMloss-val_loss0530')
    plt.close()
...
def Get_tensors(pair):
    input_tensor = tensorFromSequence(input_seq, pair[0])
    target_tensor = tensorFromSequence(output_seq, pair[1])
    protein_tensor = tensorFromSequence(protein_seq,pair[2])
...
def train(mol_tensor, protein_tensor, target_tensor,
    ...
    for ei in range(mol_length):
        mol_output, encoder_mol_hidden = encoder_mol(mol_tensor[ei], encoder_mol_hidden)
    ...
    for pi in range(pro_length):
        ...
        # encoder_pro_output = (1, 1, hidden_size))
        # encoder_pro_hidden = hn, cn = (1, num_layers_pro,  pro_hidden_size)
    ...
    decoder_hidden = merge_hiddens(encoder_mol_hidden, encoder_pro_hidden)
...
def trainiters(pairs, encoder, encoder_pro, decoder, epoches, plot_loss=500, seed=1, learning_rate=0.005):
    ...
    #print(train_pairs)
    '''
    file_object_train = open('0512pro_train_dataset.txt', 'a', encoding='utf-8')
    for ti in range(len(train_pairs)):
        file_object_train.writelines(FlitBack(train_pairs[ti][1]) + '\t')
        file_object_train.writelines(FlitBack(train_pairs[ti][0]) + '\t')
        file_object_train.writelines(FlitBack(train_pairs[ti][2]) + '\n')

    file_object_val = open('0512pro_validate_dataset.txt', 'a', encoding='utf-8')
    for vi in range(len(val_pairs)):
        file_object_val.writelines(FlitBack(val_pairs[vi][1]) + '\t')
        file_object_val.writelines(FlitBack(val_pairs[vi][0]) + '\t')
        file_object_val.writelines(FlitBack(val_pairs[vi][2]) + '\n')

    file_object_test = open('0512pro_test_dataset.txt', 'a', encoding='utf-8')
    for tei in range(len(test_pairs)):
        file_object_test.writelines(FlitBack(test_pairs[tei][1]) + '\t')
        file_object_test.writelines(FlitBack(test_pairs[tei][0]) + '\t')
        file_object_test.writelines(FlitBack(test_pairs[tei][2]) + '\n')
    '''
    ...
    # 分别画出两个loss曲线图
    torch.save(encoder.state_dict(), 'results/encoder1014.pth')
    torch.save(encoder_pro.state_dict(), 'results/encoder_pro1014.pth')
    torch.save(decoder.state_dict(), 'results/decoder1014.pth')
    # 保存训练后模型
...
def translate(pair, output):
    ...
    file_object = open('results/LSTMresults0530.txt', 'a', encoding='utf-8')
