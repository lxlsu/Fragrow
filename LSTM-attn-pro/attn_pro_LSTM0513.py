...
def showPlot(loss, axix):
    ...
    plt.show()
    #plt.close()
    plt.savefig('results/05132attnBiLSTM-loss')
    plt.close()
...
def showPlot_val(loss, val_loss, axix):
    ...
    plt.ylabel('Loss')
    plt.show()
    plt.savefig('results/05132loss-val_loss')
    plt.close()
...
def train(input_tensor, protein_tensor, target_tensor,
    ...
    encoder_fin_hidden = encoder_hidden
    encoder_pro_fin_hidden = encoder_pro_hidden
...
def trainiters(pairs, encoder, encoder_pro, decoder,  epoches, plot_every=500,
    ...
    torch.save(encoder.state_dict(), 'results/05132encoder.pth')
    torch.save(encoder_pro.state_dict(), 'results/05132encoder_pro.pth')
    torch.save(decoder.state_dict(), 'results/05132decoder.pth')
...
def translate(pair, output):
    file_object = open('results/05132AttnProBiLSTMresults.txt', 'a', encoding='utf-8')
...
def visual_attention(input_sentence, output_words, attentions, i, mode):
    ...
    plt.show()
    plt.savefig('results/05132attention_proattn_%s_%s' % (mode, i))
    plt.close()
